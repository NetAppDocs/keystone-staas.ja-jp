...
.. _Keystone Billing_ 公開証明書を次の場所からダウンロードします。 https://keystone.netapp.com/downloads/OVA-SSL-NetApp-Keystone-20251020.pem["リンクをクリックしてください"^]。
...
.. Sectigoからコード署名CA証明書をダウンロードする https://comodoca.my.salesforce.com/sfc/p/1N000002Ljih/a/3l000000oAhy/QCCby12C7cYo50nNyic6AuG1KFcwe1rDn1EknfTaUzY["こちらをご覧ください"^]。
.. の手順に従います `Resolution` このナレッジベース（KB）記事のセクション： https://kb.vmware.com/s/article/84240[]。


====
+
Keystone Collector OVAの完全性と信頼性が検証されると、テキストが表示されます。 `(Trusted certificate)` 出版社と。 image:ova-deploy-1.png["検証済みの証明書が表示されるOVA UI"]
