...
NOTE: Keystone サブスクリプションの容量消費量は、ダッシュボードとレポートに TiBs で表示され、小数点以下第 2 位まで四捨五入されます。使用量が 0.01 TiB 未満の場合、値は 0 または「使用量なし」と表示されます。これらの画面のデータは、UTC 時間（サーバーのタイムゾーン）で表示されます。クエリの日付を入力すると、自動的に UTC 時間として扱われます。使用状況指標の詳細については、link:../concepts/metrics.html#metrics-measurement["メトリック測定"]を参照してください。Keystone で使用されるさまざまな容量については、link:../concepts/supported-storage-capacity.html["Keystoneのストレージ容量タイプ"]を参照してください。
