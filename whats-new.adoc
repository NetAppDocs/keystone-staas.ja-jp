...
購読状況を示すタイムラインチャートでは、異なる種類の購読更新が同日に複数発生した場合、個別のアイコンではなく数値が表示されるようになりました。その数字にカーソルを合わせると、その日のすべての更新の種類が表示されます。ダウンロードしたレポートには、更新内容ごとに個別の行が含まれており、すべての変更履歴を完全に確認できます。詳細については、 https://docs.netapp.com/us-en/keystone-staas/integrations/subscription-timeline.html["Keystoneサブスクリプションのタイムラインを表示する"]を参照してください。
...
*消費動向*タブの履歴データ消費量グラフで、バースト制限を超える使用量が強調表示されるようになりました。詳細については、 https://docs.netapp.com/us-en/keystone-staas/integrations/consumption-tab.html["Keystoneサブスクリプションの消費傾向を表示する"]を参照してください。
...
*消費傾向*タブの*履歴データ*ビューにある*予測*トグルを使用すると、現在の日付から今後120日間の容量使用量を予測できます。詳細については、 https://docs.netapp.com/us-en/keystone-staas/integrations/consumption-tab.html#view-forecast-data["予測データを表示する"]を参照してください。
...
Keystoneバージョン 3 (v3) をサブスクライブするには、 Keystoneサポート チームにお問い合わせください。詳細については、 link:https://docs.netapp.com/us-en/keystone-staas/concepts/support-services.html["Keystoneに関するサポートを受ける"]。
